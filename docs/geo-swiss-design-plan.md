...
|---|---|---|---|
| Access | aplat « coffre » sur toute la largeur, décalé de deux colonnes : l'énoncé (première phrase du lead, corps d'affiche) démarre avant lui et change de couleur en le traversant ; une photo de cadenas sur clavier (Unsplash, choisie par Mihai, bleu gardé) sur les trois dernières colonnes, collée à l'aplat ; le titre « Vos accès » a sauté, l'énoncé est le h2 ; journal « quand / qui / pour quoi » et message d'arrêt + état en 6/6 ; deux régimes de plateformes 6/6, CMS en typographie | l'aplat du coffre | `text` réduit à une phrase ; `items.log.text`, `items.revoke.text` réécrits ; nouveaux `vaultNote`, `vaultLink` (le coffre est nommé : Infisical, avec un lien), `log`, `revoke`, `regimes`. `platforms` et `cms.*.note` non lus |
