...
|---|---|---|---|
| Access | aplat « coffre » sur toute la largeur, décalé de deux colonnes : l'énoncé (première phrase du lead, corps d'affiche) démarre avant lui et change de couleur en le traversant ; champ masqué en pied ; journal « quand / qui / pour quoi » et message d'arrêt + état en 6/6 ; deux régimes de plateformes 6/6, CMS en typographie | l'aplat du coffre | `text` réduit à une phrase ; `items.log.text`, `items.revoke.text` réécrits ; nouveaux `vaultField`, `log`, `revoke`, `regimes`. `platforms` et `cms.*.note` non lus |
